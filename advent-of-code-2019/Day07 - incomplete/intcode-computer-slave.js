...
function run(program, pointer, input) {
    let position, value, offset, nextPointer;
    inputOK = false;
    

    programInput = input;

    [pointer, offset, nextPointer] = getInstruction(program, pointer, program[pointer]);
    ...
    while (program[pointer] !== OP_HALT) {
        ...
        if (typeof (value) !== 'null') {
            if(position === 'externalProcedure') {
                return [value, nextPointer, program];
            } else if (position === 'waitingInput') {
                return [lastValidValue, pointer, program];
            }
    ...
    return [value, nextPointer, 99];
}


function applyOpCode(opcode, puzzleInput, pointer) {
